...
class IngestService:
    ...
    def ingest_test_pdf_chunks(self, file_name: str, file_data: AnyStr | Path) -> list[IngestedDoc]:
        ...
        pdftotext_text = "\n\n".join(pdf)
        text_splitter = CharacterTextSplitter(chunk_size=1024, chunk_overlap=40)
        chunked_documents = text_splitter.split_text(pdftotext_text)
        logger.info(
            "Transformed file=%s into count=%s documents", file_name, len(chunked_documents)
        )
